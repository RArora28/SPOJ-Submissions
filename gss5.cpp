...
node merge(node c1, node c2) {
  ...
  p.ans = max(c1.ans, max(c2.ans, c1.right + c2.left));
  return p;
}
...
node query(int arr[], node tree[], int low, int high, int l, int r, int pos) {  
  if(l <= low && r >= high)
    ...
    node temp;
    temp.left = temp.right = temp.sum = temp.ans = 0;
    return temp;
  }
...
int main() {
  ...
  while(t--) {
	...
	node temp1 = query(arr, tree, 1, n, x2, y1, 1);
	
	int ans = temp1.ans;

	if(temp1.ans == temp1.left) {
	  node temp2 = query(arr, tree, 1, n, x1, x2-1, 1);
	  ans = max(ans, ans + temp2.right);
	}

	if(temp1.ans == temp1.right) {
	  node temp3 = query(arr, tree, 1, n, y1+1, y2, 1);
	  ans = max(ans, ans + temp3.left);
	}
	pi(ans);
      }
    }
